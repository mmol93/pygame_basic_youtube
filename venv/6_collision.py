...
enemy_size = enemy.get_rect().size    # 그림의 크기 가져오기
...
enemy_y_pos = screen_height / 2 - enemy_height / 2      # 화면 가로의 절반 크기에 해당하는 곳에 위치
...
while running:
    ...
    # 케릭터의 x좌표(케릭터 기준 좌측)을 지속적으로 업데이트
    ...
    screen.blit(chracter, (chracter_x_pos, chracter_y_pos)) # 케릭터 그리기
